...
interface RecurringExpenseFormDialogProps {
  ...
  onUpdate: (id: string, input: RecurringExpenseUpdateInput) => Promise<void>
}
...
export function RecurringExpenseFormDialog({
  ...
  const expenseCategories = useMemo(
    () => categories.filter((item) => item.type === 'expense' && item.isActive),
    [categories],
  )
  ...
  const [endMonth, setEndMonth] = useState(expense?.endMonth ?? '')
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(caught instanceof ApiError ? caught.message : 'Could not save this fixed cost.')
      setSaving(false)
    }
  }
  ...
  return (
    <Modal
      label={mode === 'create' ? 'Add fixed cost' : 'Edit fixed cost'}
      onClose={onClose}
      dismissible={!saving}
    >
      <form onSubmit={handleSubmit} noValidate>
        <h2 className="text-lg font-semibold text-neutral-900 dark:text-neutral-50">
          {mode === 'create' ? 'Add monthly fixed cost' : 'Edit monthly fixed cost'}
        </h2>
        <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
          {mode === 'create'
            ? 'This is a scheduled monthly obligation, not a payment confirmation.'
        ...
        <div className="mt-6 grid gap-4">
          ...
          <label className="block text-sm">
            <span className="mb-1.5 block font-medium text-neutral-700 dark:text-neutral-300">Due day</span>
            <input
              type="number"
              min={1}
              max={31}
            ...
            <>
              ...
              {historicalWarning && (
                <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-200">
                  This changes historical Cash Flow and may cause your Saving Pot to reconcile the affected closed month.
                </p>
              )}
            </>
          )}
        ...
        <div className="mt-7 flex justify-end gap-3">
          <button
            ...
            onClick={onClose}
            disabled={saving}
            className="rounded-full px-4 py-2 text-sm font-medium text-neutral-600 transition hover:bg-neutral-100 disabled:opacity-50 dark:text-neutral-300 dark:hover:bg-neutral-800"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className="rounded-full bg-violet-600 px-5 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50 dark:bg-violet-500 dark:hover:bg-violet-400"
          >
            {saving ? 'Saving…' : mode === 'create' ? 'Add fixed cost' : 'Save changes'}
          </button>
        </div>
      </form>
    </Modal>
